...
// Customize color
// ----------------------------------------
const getBaseColorSelect = (): HTMLSelectElement => {
  return document.querySelector('#base_color') as HTMLSelectElement;
};

const setBaseColorSelectOptions = (): void => {
  const baseColors = Object.keys(BASE_COLOR_MAP);
  const html = baseColors
    .map((color, index) => {
    ...
    .join('\n');
  getBaseColorSelect().innerHTML = html;
};
...
const handleCustomizeButtonClick = (): void => {
  const options: PleaseOptions = {
    ...defaultOptions,
    base_color: getBaseColorSelect().value
  };
